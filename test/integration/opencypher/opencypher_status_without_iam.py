...
class TestOpenCypherStatusWithoutIam(DataDrivenOpenCypherTest):
    ...
    def setUp(self) -> None:
        res = do_opencypher_status(self.host, self.port, self.ssl, self.auth_mode, self.request_generator)
    ...
    def test_do_opencypher_status_nonexistent(self):
        ...
        with self.assertRaises(HTTPError) as error:
            do_opencypher_status(self.host, self.port, self.ssl, self.auth_mode, request_generator, query_id)
    ...
    def test_do_opencypher_cancel_nonexistent(self):
        ...
        request_generator = create_request_generator(AuthModeEnum.DEFAULT)
        with self.assertRaises(HTTPError) as error:
            do_opencypher_cancel(self.host, self.port, self.ssl, self.auth_mode, request_generator, query_id, False)
        err = json.loads(error.exception.response.content.decode('utf-8'))
        self.assertEqual(err['code'], "InvalidParameterException")
        expected_message = f'Supplied queryId {query_id} is invalid'
        self.assertEqual(err['detailedMessage'], expected_message)
    ...
    def test_do_opencypher_cancel_empty_query_id(self):
        with self.assertRaises(ValueError):
            query_id = ''
            request_generator = create_request_generator(AuthModeEnum.DEFAULT)
            do_opencypher_cancel(query_id, False, self.host, self.port, self.ssl, request_generator)
    ...
    def test_do_opencypher_status_and_cancel(self):
        ...
        query_id = ''
        request_generator = create_request_generator(AuthModeEnum.DEFAULT)
        status_res = do_opencypher_status(self.host, self.port, self.ssl, self.auth_mode, request_generator, query_id)
        ...
        cancel_res = do_opencypher_cancel(self.host, self.port, self.ssl, self.auth_mode, request_generator, query_id,
                                          False)
