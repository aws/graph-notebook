...
from graph_notebook.neptune.client import (
    ...
    DEFAULT_PORT,
    DEFAULT_REGION,
    DEFAULT_GREMLIN_SERIALIZER,
    DEFAULT_GREMLIN_TRAVERSAL_SOURCE,
...
class SparqlSection(object):
    ...
    def __init__(self, path: str = SPARQL_ACTION, endpoint_prefix: str = ""):
        ...
        if endpoint_prefix != "":
            print(
                "endpoint_prefix has been deprecated and will be removed in version 2.0.20 or greater."
            )
            if path == "":
...
class GremlinSection(object):
    ...
    def __init__(
        ...
        if serializer_lower == "":
            ...
            print(
                f"Invalid Gremlin serializer specified, defaulting to graphsonv3. "
...
class Configuration(object):
    def __init__(
        ...
        gremlin_section: GremlinSection = None,
        ...
        is_neptune_host = is_allowed_neptune_host(
            hostname=self.host, host_allowlist=neptune_hosts
        ) or is_allowed_neptune_host(
            hostname=self.proxy_host, host_allowlist=neptune_hosts
        )
        if is_neptune_host:
            ...
            self.gremlin = (
                gremlin_section if gremlin_section is not None else GremlinSection()
            )
            self.neo4j = neo4j_section if neo4j_section is not None else Neo4JSection()
            self.memgraph = (
                memgraph_section if memgraph_section is not None else MemgraphSection()
            )
...
def generate_config(
    ...
    proxy_host: str = "",
    proxy_port: int = DEFAULT_PORT,
    sparql_section: SparqlSection = SparqlSection(),
    gremlin_section: GremlinSection = GremlinSection(),
    neo4j_section=Neo4JSection(),
    memgraph_section=MemgraphSection(),
    neptune_hosts: list = NEPTUNE_CONFIG_HOST_IDENTIFIERS,
):
...
def generate_default_config():
    c = generate_config(
        "change-me", 8182, AuthModeEnum.DEFAULT, True, True, "", DEFAULT_REGION
    )
    return c
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--host", help="the host url to form a connection with", required=True
    )
    parser.add_argument(
        "--port", help="the port to use when creating a connection", default=8182
    )
    parser.add_argument(
        "--auth_mode",
        default=AuthModeEnum.DEFAULT.value,
    ...
    parser.add_argument(
        "--config_destination",
        help="location to put generated config",
        default=DEFAULT_CONFIG_LOCATION,
    )
    parser.add_argument(
        "--load_from_s3_arn", help="arn of role to use for bulk loader", default=""
    )
    parser.add_argument(
        "--aws_region", help="aws region your ml cluster is in.", default=DEFAULT_REGION
    )
    parser.add_argument(
        "--proxy_host",
        help="the proxy host url to route a connection through",
        default="",
    )
    parser.add_argument(
        "--proxy_port",
        help="the proxy port to use when creating proxy connection",
        default=8182,
    )
    parser.add_argument(
        "--neptune_hosts",
    ...
    parser.add_argument(
        "--sparql_path",
        help="the namespace path to append to the SPARQL endpoint",
        default=SPARQL_ACTION,
    )
    parser.add_argument(
        "--gremlin_traversal_source",
        help="the traversal source to use for Gremlin queries",
        default=DEFAULT_GREMLIN_TRAVERSAL_SOURCE,
    )
    parser.add_argument(
        "--gremlin_username",
        help="the username to use when creating Gremlin connections",
        default="",
    )
    parser.add_argument(
        "--gremlin_password",
        help="the password to use when creating Gremlin connections",
        default="",
    )
    parser.add_argument(
        "--gremlin_serializer",
    ...
    parser.add_argument(
        "--neo4j_username",
        help="the username to use for Neo4J connections",
        default=DEFAULT_NEO4J_USERNAME,
    )
    parser.add_argument(
        "--neo4j_password",
        help="the password to use for Neo4J connections",
        default=DEFAULT_NEO4J_PASSWORD,
    )
    parser.add_argument(
        "--neo4j_auth",
        help="whether to use auth for Neo4J connections or not [True|False]",
        default=True,
    )
    parser.add_argument(
        "--neo4j_database",
        help="the name of the database to use for Neo4J",
        default=DEFAULT_NEO4J_DATABASE,
    )
    parser.add_argument(
        "--memgraph_username",
        help="the username to use for Memgraph connections",
        default=DEFAULT_MEMGRAPH_USERNAME,
    )
    parser.add_argument(
        "--memgraph_password",
        help="the password to use for Memgraph connections",
    ...
    auth_mode_arg = (
        args.auth_mode if args.auth_mode != "" else AuthModeEnum.DEFAULT.value
    )
    config = generate_config(
        ...
        args.ssl,
        args.ssl_verify,
        args.load_from_s3_arn,
        args.aws_region,
        args.proxy_host,
        int(args.proxy_port),
        SparqlSection(args.sparql_path, ""),
        GremlinSection(
            args.gremlin_traversal_source,
            args.gremlin_username,
            args.gremlin_password,
        ...
        MemgraphSection(
            args.memgraph_username, args.memgraph_password, args.memgraph_auth
        ),
        args.neptune_hosts,
    )
