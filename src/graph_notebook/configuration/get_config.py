...
def get_config_from_dict(
    data: dict, neptune_hosts: list = NEPTUNE_CONFIG_HOST_IDENTIFIERS
) -> Configuration:
    if "ssl_verify" in data:
        ...
        ssl_verify = True
    sparql_section = (
        SparqlSection(**data["sparql"]) if "sparql" in data else SparqlSection("")
    )
    gremlin_section = (
        GremlinSection(**data["gremlin"]) if "gremlin" in data else GremlinSection()
    )
    neo4j_section = (
        Neo4JSection(**data["neo4j"])
        if "neo4j" in data
        else Neo4JSection("", "", True, "")
    )
    memgraph_section = (
        MemgraphSection(**data["memgraph"])
        if "memgraph" in data
        else MemgraphSection("", "", False)
    )
    ...
    is_neptune_host = is_allowed_neptune_host(
        hostname=data["host"], host_allowlist=neptune_hosts
    )
    ...
    if is_neptune_host:
        ...
        if (
            neo4j_section.to_dict()["username"] != DEFAULT_NEO4J_USERNAME
            or neo4j_section.to_dict()["password"] != DEFAULT_NEO4J_PASSWORD
        ):
            print(
                "Ignoring Neo4J custom authentication, Amazon Neptune does not support this functionality.\n"
            )
        if neo4j_section.to_dict()["database"] != DEFAULT_NEO4J_DATABASE:
            print(
                "Ignoring Neo4J custom database, Amazon Neptune does not support multiple databases.\n"
            )
        if (
            memgraph_section.to_dict()["username"] != DEFAULT_MEMGRAPH_USERNAME
        ...
        config = Configuration(
            ...
            neo4j_section=neo4j_section,
            memgraph_section=memgraph_section,
        ...
        if excluded_params:
            print(
                f"The provided configuration contains the following parameters that are incompatible with the "
...
def get_config(
    path: str = DEFAULT_CONFIG_LOCATION,
