...
from graph_notebook.configuration.generate_config import (
...
from graph_notebook.magics.ml import (
    neptune_ml_magic_handler,
    generate_neptune_ml_parser,
)
...
from graph_notebook.neptune.client import (
    ...
    STATISTICS_LANGUAGE_INPUTS,
    STATISTICS_MODES,
    SUMMARY_MODES,
    SPARQL_EXPLAIN_MODES,
    OPENCYPHER_EXPLAIN_MODES,
)
from graph_notebook.network import SPARQLNetwork
from graph_notebook.network.gremlin.GremlinNetwork import (
    parse_pattern_list_str,
    GremlinNetwork,
)
from graph_notebook.visualization.rows_and_columns import (
...
from graph_notebook.seed.load_query import (
    get_data_sets,
    get_queries,
...
from graph_notebook.magics.metadata import (
...
DEFAULT_LAYOUT = widgets.Layout(
    max_height="600px", max_width="940px", overflow="scroll"
)
...
NEPTUNE_RDF_SELECT_FORMATS = [
    MEDIA_TYPE_SPARQL_JSON,
    MEDIA_TYPE_SPARQL_XML,
    MEDIA_TYPE_BINARY_RESULTS_TABLE,
    MEDIA_TYPE_SPARQL_CSV,
    MEDIA_TYPE_SPARQL_TSV,
]
NEPTUNE_RDF_ASK_FORMATS = [
    MEDIA_TYPE_SPARQL_JSON,
    MEDIA_TYPE_SPARQL_XML,
    MEDIA_TYPE_BOOLEAN,
]
NEPTUNE_RDF_CONSTRUCT_DESCRIBE_FORMATS = [
    MEDIA_TYPE_SPARQL_JSON,
    MEDIA_TYPE_NQUADS,
...
def process_statistics_400(is_summary: bool, response):
    ...
    if res_code == "StatisticsNotAvailableException":
        print(
            "No statistics found. Please ensure that auto-generation of DFE statistics is enabled by running "
        ...
        print(
            "Unable to query the statistics endpoint. Please check that your Neptune instance is of size r5.large or "
            "greater in order to have DFE statistics enabled."
        )
        if (
            is_summary
            and "Statistics is disabled" not in bad_request_res["detailedMessage"]
        ):
            print(
...
class Graph(Magics):
    def __init__(self, shell):
        ...
        try:
            self.config_location = os.getenv(
                "GRAPH_NOTEBOOK_CONFIG", DEFAULT_CONFIG_LOCATION
            )
            self.client: Client = None
            self.graph_notebook_config = get_config(
                self.config_location, neptune_hosts=self.neptune_cfg_allowlist
            )
    ...
    def _generate_client_from_config(self, config: Configuration):
        ...
        is_neptune_host = is_allowed_neptune_host(
            hostname=config.host, host_allowlist=self.neptune_cfg_allowlist
        )
    ...
    def graph_notebook_config(self, line="", cell="", local_ns: dict = None):
        ...
        if cell != "":
            ...
            config = get_config_from_dict(
                data, neptune_hosts=self.neptune_cfg_allowlist
            )
        ...
        store_to_ns(
            args.store_to,
            json.dumps(self.graph_notebook_config.to_dict(), indent=2),
            local_ns,
        )
        ...
        if args.mode == "reset":
            ...
            if args.mode == "add":
                if input_type == "string":
                    ...
                    self.neptune_cfg_allowlist = list(
                        set(self.neptune_cfg_allowlist) | set(cell_new)
                    )
    ...
    def stream_viewer(self, line):
        ...
        parser.add_argument(
            ...
            type=int,
            default=10,
            help="Maximum number of rows to display at a time",
        )
    ...
    def statistics(self, line, local_ns: dict = None):
        ...
        parser.add_argument(
            ...
            action="store_true",
            default=False,
            help="Retrieves the graph summary.",
        )
        parser.add_argument(
            "--silent", action="store_true", default=False, help="Display no output."
        )
        ...
        elif (args.summary and mode not in SUMMARY_MODES) or (
            not args.summary and mode not in STATISTICS_MODES
        ):
            err_endpoint_type, err_mode_list, err_default_mode = (
                ("summary", SUMMARY_MODES[1:], "basic summary view")
                if args.summary
                else ("statistics", STATISTICS_MODES[1:], "status")
            )
            print(
                f"Invalid {err_endpoint_type} mode. Please specify one of: {err_mode_list}, "
    ...
    def sparql(self, line="", cell="", local_ns: dict = None):
        ...
        parser.add_argument(
            "--explain-format",
            default="text/html",
            help="response format for explain query mode",
        ...
        parser.add_argument(
            ...
            action="store_true",
            default=False,
            help="Ignore all grouping options",
        )
        parser.add_argument(
            "-sp",
            "--stop-physics",
        ...
        if mode == QueryMode.EXPLAIN:
            res = self.client.sparql_explain(
                cell, args.explain_type, args.explain_format, path=path
            )
            ...
            if not args.silent:
                sparql_metadata = build_sparql_metadata_from_query(
                    query_type="explain", res=res
                )
                titles.append("Explain")
                explain_bytes = explain.encode("ascii", "ignore")
                base64_str = base64.b64encode(explain_bytes).decode("ascii")
                first_tab_html = sparql_explain_template.render(
                    table=explain, link=f"data:text/html;base64,{base64_str}"
                )
        else:
            ...
            if is_allowed_neptune_host(
                self.graph_notebook_config.host, NEPTUNE_CONFIG_HOST_IDENTIFIERS
            ):
                if (
                    (
                        query_type == "SELECT"
                        and result_type not in NEPTUNE_RDF_SELECT_FORMATS
                    )
                    or (
                        query_type == "ASK"
                        and result_type not in NEPTUNE_RDF_ASK_FORMATS
                    )
                    or (
                        query_type in ["CONSTRUCT", "DESCRIBE"]
                    ...
                    if result_type != "":
                        print(
                            f"Invalid media type: {result_type} specified for Neptune {query_type} query. "
            ...
            try:
                ...
                result_type = (
                    MEDIA_TYPE_SPARQL_JSON
                    if query_type == "SELECT"
                    else MEDIA_TYPE_NTRIPLES
                )
                query_res = self.client.sparql(
                    cell, path=path, headers={"Accept": result_type}
                )
                query_res.raise_for_status()
            ...
            if not args.silent:
                ...
                if result_type != MEDIA_TYPE_SPARQL_JSON:
                    ...
                    if query_type in ["SELECT", "CONSTRUCT", "DESCRIBE"]:
                        ...
                        sn = SPARQLNetwork(
                            group_by_property=args.group_by,
                        ...
                        if len(sn.graph.nodes) > 0:
                            self.graph_notebook_vis_options["physics"][
                            ...
                            f = Force(
                                network=sn, options=self.graph_notebook_vis_options
                            )
                            titles.append("Graph")
                            children.append(f)
                        ...
                        if rows_and_columns is not None:
                            ...
                            visible_results = results_per_page_check(
                                args.results_per_page
                            )
                            first_tab_html = sparql_table_template.render(
                ...
                sparql_metadata = build_sparql_metadata_from_query(
                    query_type="query", res=query_res, results=results
                )
        ...
        if not args.silent:
            ...
            if first_tab_html == "" and results_df is None:
                tab.children = children[
                    1:
                ]  # the first tab is empty, remove it and proceed
            else:
                tab.children = children
            ...
            if results_df is not None:
                with first_tab_output:
                    (
                        visible_results,
                        final_pagination_options,
                        final_pagination_menu,
                    ) = generate_pagination_vars(args.results_per_page)
                    sparql_columndefs = [
                        {"width": "5%", "targets": 0},
                        ...
                        {
                            "createdCell": JavascriptFunction(cell_style_js),
                            "targets": "_all",
                        },
                    ]
                    if args.hide_index:
    ...
    def gremlin(self, line, cell, local_ns: dict = None):
        ...
        if mode == QueryMode.EXPLAIN:
            res = self.client.gremlin_explain(
                cell,
            ...
            if not args.silent:
                gremlin_metadata = build_gremlin_metadata_from_query(
                    query_type="explain", results=query_res, res=res
                )
                titles.append("Explain")
                if "Neptune Gremlin Explain" in query_res:
                    explain_bytes = query_res.encode("ascii", "ignore")
                    base64_str = base64.b64encode(explain_bytes).decode("ascii")
                    first_tab_html = gremlin_explain_profile_template.render(
                        content=query_res, link=f"data:text/html;base64,{base64_str}"
                    )
                else:
                    first_tab_html = pre_container_template.render(
            ...
            profile_args = {
                "profile.results": args.profile_no_results,
            ...
            try:
                ...
                print(
                    "--profile-misc-args received invalid input, please check that you are passing in a valid "
            ...
            if not args.silent:
                gremlin_metadata = build_gremlin_metadata_from_query(
                    query_type="profile", results=query_res, res=res
                )
            ...
            query_start = (
                time.time() * 1000
            )  # time.time() returns time in seconds w/high precision; x1000 to get in ms
            query_res = self.client.gremlin_query(cell, transport_args=transport_args)
            query_time = time.time() * 1000 - query_start
            if not args.silent:
                gremlin_metadata = build_gremlin_metadata_from_query(
                    query_type="query", results=query_res, query_time=query_time
                )
    ...
    def gremlin_status(self, line="", local_ns: dict = None):
        ...
        if not args.cancelQuery:
            status_res = self.client.gremlin_status(
                query_id=args.queryId, include_waiting=args.includeWaiting
            )
            status_res.raise_for_status()
            res = status_res.json()
        else:
        ...
        try:
            ...
            if not args.silent:
                if "blazegraph&trade; by SYSTAP" in status_res.text:
                    print(
                        "For more information on the status of your Blazegraph cluster, please visit: "
                    )
                    print()
                    print(
                        f"http://{self.graph_notebook_config.host}:{self.graph_notebook_config.port}"
                        f"/blazegraph/#status"
                    )
                    print()
                return status_res
    ...
    def db_reset(self, line, local_ns: dict = None):
        ...
        parser.add_argument(
            "-t", "--token", default="", help="perform database reset with given token"
        )
        parser.add_argument(
            "-y",
            "--yes",
        ...
        max_status_retries = (
            args.max_status_retries if args.max_status_retries > 0 else 1
        )
            ...
            def on_button_delete_clicked(b):
                ...
                with output:
                    ...
                    if interval_check_response.get("status") != "healthy":
                        print(
                            f"Could not retrieve the status of the reset operation within the allotted time of "
    ...
    def load(self, line="", local_ns: dict = None):
        ...
        try:
            ...
            print(
                f"Missing required configuration option 'load_from_s3_arn'. Please ensure that you have provided a "
                "valid Neptune cluster endpoint URI in the 'host' field of %graph_notebook_config."
            )
            return
        parser.add_argument(
            "-f", "--format", choices=LOADER_FORMAT_CHOICES, default=FORMAT_CSV
        )
        parser.add_argument(
            "-p", "--parallelism", choices=PARALLELISM_OPTIONS, default=PARALLELISM_HIGH
        )
        try:
            parser.add_argument(
                "-r", "--region", default=self.graph_notebook_config.aws_region
            )
        except AttributeError:
            print(
                "Missing required configuration option 'aws_region'. Please ensure that you have provided a "
                "valid Neptune cluster endpoint URI in the 'host' field of %graph_notebook_config."
            )
            return
        parser.add_argument("--fail-on-failure", action="store_true", default=False)
        parser.add_argument(
            "--update-single-cardinality", action="store_true", default=True
        )
        parser.add_argument(
            "--store-to",
            type=str,
        ...
        parser.add_argument(
            "--named-graph-uri",
            type=str,
            default=DEFAULT_NAMEDGRAPH_URI,
        ...
        allow_empty_strings = widgets.Dropdown(
            ...
            layout=widgets.Layout(
                display=gremlin_parser_options_hbox_visibility, width=widget_width
            ),
        )
        ...
        named_graph_uri = widgets.Text(
            ...
            disabled=False,
            layout=widgets.Layout(
                display=named_graph_hbox_visibility, width=widget_width
            ),
        )
        ...
        base_uri = widgets.Text(
            ...
            disabled=False,
            layout=widgets.Layout(display=base_uri_hbox_visibility, width=widget_width),
        )
        ...
        user_provided_edge_ids = widgets.Dropdown(
            ...
            disabled=False,
            layout=widgets.Layout(display=ids_hbox_visibility, width=widget_width),
        )
        ...
        source_hbox_label = widgets.Label(
            "Source:",
            layout=widgets.Layout(
                width=label_width, display="flex", justify_content="flex-end"
            ),
        )

        source_hbox = widgets.HBox([source_hbox_label, source])

        format_hbox_label = widgets.Label(
            "Format:",
            layout=widgets.Layout(
                width=label_width, display="flex", justify_content="flex-end"
            ),
        ...
        allow_empty_strings_hbox_label = widgets.Label(
            "Allow Empty Strings:",
            layout=widgets.Layout(
                width=label_width,
        ...
        allow_empty_strings_hbox = widgets.HBox(
            [allow_empty_strings_hbox_label, allow_empty_strings]
        )

        named_graph_uri_hbox_label = widgets.Label(
            "Named Graph URI:",
            layout=widgets.Layout(
                width=label_width,
        ...
        named_graph_uri_hbox = widgets.HBox(
            [named_graph_uri_hbox_label, named_graph_uri]
        )

        base_uri_hbox_label = widgets.Label(
            "Base URI:",
            layout=widgets.Layout(
                width=label_width,
        ...
        cardinality_hbox = widgets.HBox(
            [
                widgets.Label(
                    "Update Single Cardinality:",
                    layout=widgets.Layout(
                        width=label_width, display="flex", justify_content="flex-end"
                    ),
                ),
        ...
        poll_status_label = widgets.Label(
            "Poll Load Status:",
            layout=widgets.Layout(
                width=label_width, display="flex", justify_content="flex-end"
            ),
        ...
        def update_parserconfig_options(change):
            ...
            allow_empty_strings_hbox_label.layout.display = (
                gremlin_parser_options_hbox_visibility
            )
            named_graph_uri.layout.display = named_graph_hbox_visibility_hbox_visibility
            named_graph_uri_hbox_label.layout.display = (
                named_graph_hbox_visibility_hbox_visibility
            )
        ...
        def on_button_clicked(b):
            ...
            allow_empty_strings.children = (
                allow_empty_strings_hbox_label,
                allow_empty_strings,
            )
            named_graph_uri_hbox.children = (
                named_graph_uri_hbox_label,
                named_graph_uri,
            )
            base_uri_hbox.children = (
                base_uri_hbox_label,
                base_uri,
            )
            dep_hbox.children = (
            ...
            if not (
                source.value.startswith("s3://") and len(source.value) > 7
            ...
            source_exp = os.path.expandvars(source.value)
            ...
            try:
                ...
                if source_format.value.lower() == FORMAT_OPENCYPHER:
                    ...
                    if named_graph_uri.value:
                        kwargs["parserConfiguration"][
                            "namedGraphUri"
                        ] = named_graph_uri.value
                    if (
                        base_uri.value
                        and source_format.value.lower() in BASE_URI_FORMATS
                    ):
                ...
                try:
                    if source.value.startswith("s3://"):
                        load_res = self.client.load(
                            str(source_exp), source_format.value, arn.value, **kwargs
                        )
                    else:
                        load_res = self.client.load(
                            str(source_exp), source_format.value, **kwargs
                        )
                except Exception as e:
                ...
                if poll_status.value == "FALSE":
                    ...
                    while True:
                        ...
                        if time_elapsed > poll_interval:
                            ...
                            new_interval = True
                            try:
                                load_status_res = self.client.load_status(
                            ...
                            with job_status_output:
                                ...
                                if (
                                    ...
                                    in FINAL_LOAD_STATUSES
                                ):
                                    execution_time = interval_check_response["payload"][
                                        "overallStatus"
                                    ]["totalTimeSpent"]
                                    if execution_time == 0:
                                        execution_time_statement = "<1 second"
                                    elif execution_time > 59:
                                        execution_time_statement = str(
                                            datetime.timedelta(seconds=execution_time)
                                        )
                                    else:
                                        execution_time_statement = (
                    ...
                    with table_output:
                        show(
                            ...
                            columnDefs=[
                                ...
                                {"className": "nowrap dt-left", "targets": "_all"},
                                {
                                    "createdCell": JavascriptFunction(index_col_js),
                                    "targets": 0,
                                },
                                {
                                    "createdCell": JavascriptFunction(cell_style_js),
    ...
    def cancel_load(self, line, local_ns: dict = None):
        parser = argparse.ArgumentParser()
        parser.add_argument(
            "load_id", nargs="?", default="", help="loader id to check status for"
        )
        parser.add_argument(
            "--all-in-queue",
            action="store_true",
    ...
    def seed(self, line, local_ns: dict = None):
        ...
        source_dropdown = widgets.Dropdown(
            options=SEED_SOURCE_OPTIONS, description="Source type:", disabled=False
        )
            ...
            if selected_source == "custom":
                ...
                if model_dropdown.value:
                    ...
                    if show_dataset:
                        ...
                        if (
                            data_set_drop_down.value
                            and data_set_drop_down.value != SEED_NO_DATASETS_FOUND_MSG
                        ):
                            submit_button.layout.visibility = "visible"
            else:
                custom_language_dropdown.layout.visibility = "hidden"
        ...
        def change_datasets_widget(samples_lang):
            ...
            if data_sets:
                data_sets.sort()
                data_set_drop_down.options = [
                    ds for ds in data_sets if ds != "__pycache__"
                ]  # being extra sure that we aren't passing __pycache__.
                data_set_drop_down.layout.visibility = "visible"
                data_set_drop_down.layout.display = "flex"
                submit_button.layout.visibility = "visible"
            else:
        ...
        def on_custom_language_value_change(change):
            ...
            if selected_language != "sparql":
                ...
                fullfile_option_dropdown.layout.display = "none"
            if (
                not seed_file_location_text.value
                and seed_file_location_text_hbox.layout.visibility == "hidden"
            ):
                seed_file_location.layout.visibility = "visible"
                seed_file_location.layout.display = "flex"
                submit_button.layout.visibility = "visible"
        ...
        def process_gremlin_query_line(query_line, line_index, q):
            ...
            if not query_line:
                logger.debug(
                    f"Skipped blank query at line {line_index + 1} in seed file {q['name']}"
                )
                return 0
            try:
                ...
                try:
                    error = json.loads(
                        gremlinEx.args[0][5:]
                    )  # remove the leading error code.
                    content = json.dumps(error, indent=2)
                except Exception:
                    content = {"error": gremlinEx}
                logger.debug(
                    f"GremlinServerError at line {line_index + 1} in seed file {q['name']}"
                )
                ...
                content = {"error": e}
                logger.debug(
                    f"Exception at line {line_index + 1} in seed file {q['name']}"
                )
                logger.debug(content)
                if args.ignore_errors:
                    return 1
        ...
        def on_button_clicked(b=None):
            ...
            if source_dropdown.value == "samples":
                ...
                if seed_file_location_text.value:
                    ...
                    if location_option_dropdown.value == "S3" and not (
                        seed_file_location_text.value.startswith("s3://")
                        ...
                        location_option_dropdown.value == "Local"
                    ...
                    if stall_with_warning:
                        seed_file_location_text_validation_label.style = (
                            DescriptionStyle(color="red")
                        )
                        seed_file_location_text_hbox.children += (
                            seed_file_location_text_validation_label,
                        )
                        return
            ...
            if custom_language_dropdown.value and filename:
                ...
                seeding_language = normalize_language_name(
                    custom_language_dropdown.value
                )
            else:
                model = normalize_model_name(model_dropdown.value)
                seeding_language = (
                    "sparql" if model == "rdf" else samples_pg_language_dropdown.value
                )
            with output:
                ...
                if model == "rdf":
                    ...
                    if fullfile_query:  # treat entire file content as one query
                        if seeding_language == "opencypher":
                            ...
                            query_status = process_gremlin_query_line(
                                q["content"], 0, q
                            )
                        if query_status == 2:
                            progress.close()
                        ...
                        for line_index, query_line in enumerate(
                            q["content"].splitlines()
                        ):
                            if seeding_language == "opencypher":
                                query_status = process_cypher_query_line(
                                    query_line, line_index, q
                                )
                            else:
                                query_status = process_gremlin_query_line(
                                    query_line, line_index, q
        ...
        samples_pg_language_dropdown.observe(
            on_samples_pg_language_value_change, names="value"
        )
        ...
        display(
            ...
            samples_pg_language_dropdown,
            data_set_drop_down,
            fullfile_option_dropdown,
            location_option_dropdown,
            seed_file_location,
            seed_file_location_text_hbox,
            submit_button,
            progress_output,
            output,
        )
        ...
        if (args.model != "" or args.language != "") and args.source == "":
            ...
            if selected_model:
                ...
                if selected_language:
                    ...
                    if (
                        args.dataset != ""
                        and args.dataset in data_set_drop_down.options
                    ):
    ...
    def toggle_traceback(self, line, local_ns: dict = None):
        ...
        print(
            f"Display of tracebacks from magics is toggled {'ON' if show_traceback else 'OFF'}."
        )
    ...
    def graph_notebook_vis_options(self, line="", cell="", local_ns: dict = None):
        ...
        parser.add_argument(
            ...
            type=str,
            default="",
            help="store visualization settings to this variable",
        )
        parser.add_argument(
            "--load-from",
            type=str,
            default="",
            help="load visualization settings from this variable",
        )
            ...
            try:
                if args.load_from:
                    try:
                        ...
                        print(
                            f"Unable to load visualization settings, variable [{args.load_from}] does not exist in "
                ...
                print(
                    f"Unable to load visualization settings, variable [{args.load_from}] is not in valid JSON "
                    f"format:\n"
                )
                print(e)
                return
            self.graph_notebook_vis_options = vis_options_merge(
                self.graph_notebook_vis_options, options_dict
            )
        ...
        store_to_ns(
            args.store_to,
            json.dumps(self.graph_notebook_vis_options, indent=2),
            local_ns,
        )
    ...
    def neptune_ml(self, line, cell="", local_ns: dict = None):
        ...
        args = parser.parse_args(line.split())
        logger.info(
            f"received call to neptune_ml with details: {args.__dict__}, cell={cell}, local_ns={local_ns}"
        )
    ...
    def handle_opencypher_query(self, line, cell, local_ns):
        ...
        parser.add_argument(
            ...
            nargs="?",
            default="query",
            help="query mode [query|bolt|explain]",
        ...
        if args.mode == "explain":
            ...
            res = self.client.opencypher_http(
                cell, explain=args.explain_type, query_params=query_params
            )
            query_time = time.time() * 1000 - query_start
            ...
            ##store_to_ns(args.store_to, explain, local_ns)
            if not args.silent:
                oc_metadata = build_opencypher_metadata_from_query(
                    query_type="explain",
                    results=None,
                    results_type="explain",
                    res=res,
                ...
                first_tab_html = opencypher_explain_template.render(
                    table=explain, link=f"data:text/html;base64,{base64_str}"
                )
            ...
            if not args.silent:
                ...
                if rows_and_columns:
                    ...
                    for col_index, col_name in enumerate(rows_and_columns["columns"]):
                        results_df.rename(
                            {results_df.columns[col_index + 1]: col_name},
                            axis="columns",
                            inplace=True,
                        )
                try:
                    ...
                    if len(gn.graph.nodes) > 0:
                        ...
                        force_graph_output = Force(
                            network=gn, options=self.graph_notebook_vis_options
                        )
                        titles.append("Graph")
                        children.append(force_graph_output)
                except (TypeError, ValueError) as network_creation_error:
                    logger.debug(
                        f"Unable to create network from result. Skipping from result set: {res}"
                    )
