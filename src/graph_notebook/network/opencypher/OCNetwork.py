...
from graph_notebook.network.EventfulNetwork import (
    EventfulNetwork,
    DEFAULT_GRP,
    DEPTH_GRP_KEY,
    DEFAULT_RAW_GRP_KEY,
...
class OCNetwork(EventfulNetwork):
        ...
        try:
            if isinstance(custom_property, dict):
                if isinstance(custom_property[title], tuple):
                    if (
                        custom_property[title][0] in props
                        and isinstance(props[custom_property[title][0]], list)
                        and len(props[custom_property[title][0]]) >= 2
                    ):
                        label = props[custom_property[title][0]][
                            custom_property[title][1]
                        ]
                    else:
                        label = title
                ...
                if (
                    isinstance(props[custom_property[0]], list)
                    and len(props[custom_property[0]]) >= 2
                ):
    ...
    def get_edge_property_value(self, data: dict, rel: dict, custom_property):
        if custom_property is not EDGE_TYPE_KEY:
            try:
                if isinstance(custom_property, dict):
                    if (
                        isinstance(custom_property[data["label"]], tuple)
                        and custom_property[data["label"]][0] in data["properties"]
                    ):
                        if (
                            isinstance(
                                data["properties"][custom_property[data["label"]][0]],
                                list,
                            )
                            and len(
                                data["properties"][custom_property[data["label"]][0]]
                        ...
                        display_label = data["properties"][
                            custom_property[rel[EDGE_TYPE_KEY]]
                        ]
                elif (
                    isinstance(custom_property, tuple)
                    and custom_property[0] in data["properties"]
                ):
                    display_label = str(
                        data["properties"][custom_property[0]][custom_property[1]]
                    )
                else:
    ...
    def parse_node(self, node: dict, path_index: int = -2):
        ...
        if not isinstance(
            self.group_by_property, dict
        ):  # Handle string format group_by
            try:
            ...
            try:
                if (
                    str(node[LABEL_KEY][0]) in self.group_by_property
                    and len(node[LABEL_KEY]) > 0
                ):
        ...
        label = self.get_node_property_value(
            node, props, title_plc, self.display_property
        )
        if not label:
            label = node[ID_KEY]
        title, label = self.strip_and_truncate_label_and_title(
            label, self.label_max_length
        )
        if self.tooltip_property and self.tooltip_property != self.display_property:
            title, label_plc = self.strip_and_truncate_label_and_title(
                self.get_node_property_value(
                    node, props, title_plc, self.tooltip_property
                )
            )
    ...
    def parse_rel(self, rel):
        data = {
            "properties": self.flatten(rel),
            "label": rel[EDGE_TYPE_KEY],
            "title": rel[EDGE_TYPE_KEY],
        }
        display_label = self.get_edge_property_value(
            data, rel, self.edge_display_property
        )
        edge_title, edge_label = self.strip_and_truncate_label_and_title(
            display_label, self.edge_label_max_length
        )
        if (
            self.edge_tooltip_property
            and self.edge_tooltip_property != self.edge_display_property
        ):
            edge_title = self.get_edge_property_value(
                data, rel, self.edge_tooltip_property
            )
        data["title"] = edge_title
        data["label"] = edge_label
        self.add_edge(
            from_id=rel[START_KEY],
            to_id=rel[END_KEY],
            edge_id=rel[ID_KEY],
            label=edge_label,
            title=edge_title,
                    ...
                    if type(res[k]) is dict:
                        ...
                        for path_index, res_sublist in enumerate(res[k]):
                            try:
                                ...
                                logger.debug(
                                    f"Property {res_sublist} in list results set is invalid, skipping"
                                )
                                logger.debug(f"Error: {e}")
                                continue

 def create_schema_network(self, data):
        try:
            schema = data["results"][0]["schema"]
            for item in schema["nodeLabels"]:
                props = {}
                for p in schema["nodeLabelDetails"][item]["properties"]:
                    props[p] = ",".join(
                        schema["nodeLabelDetails"][item]["properties"][p]["datatypes"]
                    )
                self.add_node(
                    node_id=item,
                    data={
                        "properties": self.flatten(props),
                        "label": item,
                        "title": item,
                    },
                )

            for item in schema["labelTriples"]:
                # if item[START_KEY] == item[END_KEY]:
                #     G.add_node(node_for_adding=item[START_KEY], label=item[START_KEY])
                # else:
                #     G.add_node(node_for_adding=item[START_KEY], label=item[START_KEY])
                #     G.add_node(node_for_adding=item[END_KEY], label=item[END_KEY])
                #     G.add_edge(from_id=item[START_KEY], to_id=item[END_KEY], label=item[LABEL_KEY])
                self.add_edge(
                    from_id=item["~from"],
                    to_id=item["~to"],
                    edge_id=f"{item['~from']}_{item['~type']}_{item['~to']}",
                    label=item["~type"],
                    title=item["~type"],
                )
            # self.add_node("foo")
