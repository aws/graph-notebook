...
class Client(object):
    def __init__(
        ...
        self.neptune_hosts = (
            NEPTUNE_CONFIG_HOST_IDENTIFIERS if neptune_hosts is None else neptune_hosts
        )
    ...
    def is_neptune_domain(self):
        return is_allowed_neptune_host(
            hostname=self.target_host, host_allowlist=self.neptune_hosts
        )
    ...
    def sparql_query(
        self, query: str, headers=None, explain: str = "", path: str = ""
    ) -> requests.Response:
        if headers is None:
            headers = {}
    ...
    def sparql_update(
        self, update: str, headers=None, explain: str = "", path: str = ""
    ) -> requests.Response:
        if headers is None:
            headers = {}
    ...
    def do_sparql_request(
        self, data: dict, headers=None, explain: str = "", path: str = ""
    ):
    ...
    def sparql_cancel(self, query_id: str, silent: bool = False):
        ...
        return self._query_status(
            "sparql", query_id=query_id, silent=silent, cancelQuery=True
        )
    ...
    def get_gremlin_connection(self, transport_kwargs) -> client.Client:
        ...
        traversal_source = (
            "g" if self.is_neptune_domain() else self.gremlin_traversal_source
        )
    ...
    def gremlin_query(self, query, transport_args=None, bindings=None):
        ...
        try:
            ...
            if isinstance(e, GremlinServerError):
                source_err = re.compile(
                    "The traversal source \\[.] for alias \\[.] is not configured on the server\\."
                )
                if e.status_code == 499 and source_err.search(str(e)):
                    print(
                        "Error returned by the Gremlin Server for the traversal_source specified in notebook "
    ...
    def opencyper_bolt(self, query: str, **kwargs):
        ...
        with driver.session(database=self.neo4j_database) as session:
            try:
                ...
                print(
                    "The Bolt request failed with an authentication error. Please ensure that the 'neo4j' or 'memgraph' section "
    ...
    def opencypher_cancel(self, query_id, silent: bool = False):
        ...
        return self._query_status(
            "openCypher", query_id=query_id, cancelQuery=True, silent=silent
        )
    ...
    def get_opencypher_driver(self):
        ...
        if self.is_neptune_domain():
            if self._session and self.iam_enabled:
                ...
                engine_version = int(
                    engine_version_raw.rsplit(".", 1)[0].replace(".", "")
                )
    ...
    def load(
        self, source: str, source_format: str, iam_role_arn: str = None, **kwargs
    ) -> requests.Response:
        """
        ...
        payload = {"source": source, "format": source_format, "region": self.region}
        ...
        req = self._prepare_request(
            "POST", url, data=raw, headers={"content-type": "application/json"}
        )
        res = self._http_session.send(req, verify=self.ssl_verify)
        return res
    ...
    def dataprocessing_start(
        self, s3_input_uri: str, s3_output_uri: str, **kwargs
    ) -> requests.Response:
        data = {
    ...
    def dataprocessing_job_status(
        self, job_id: str, neptune_iam_role_arn: str = ""
    ) -> requests.Response:
    ...
    def dataprocessing_list(
        self, max_items: int = 10, neptune_iam_role_arn: str = ""
    ) -> requests.Response:
        url = f"{self._http_protocol}://{self.host}:{self.port}/ml/dataprocessing"
    ...
    def dataprocessing_stop(
        self, job_id: str, clean=False, neptune_iam_role_arn: str = ""
    ) -> requests.Response:
        url = f"{self._http_protocol}://{self.host}:{self.port}/ml/dataprocessing/{job_id}"
        data = {"clean": clean}
        if neptune_iam_role_arn != "":
    ...
    def modeltraining_start(
        self,
        data_processing_job_id: str,
        train_model_s3_location: str,
        max_hpo_number_of_training_jobs: int,
        max_hpo_parallel_training_jobs: int,
        **kwargs,
    ) -> requests.Response:
        """
    ...
    def modeltraining_job_status(
        self, training_job_id: str, neptune_iam_role_arn: str = ""
    ) -> requests.Response:
        data = (
            {}
            if neptune_iam_role_arn == ""
            else {"neptuneIamRoleArn": neptune_iam_role_arn}
        )
    ...
    def modeltraining_stop(
        self, training_job_id: str, neptune_iam_role_arn: str = "", clean: bool = False
    ) -> requests.Response:
        data = {
    ...
    def modeltransform_create(
        self,
        output_s3_location: str,
        dataprocessing_job_id: str = "",
        modeltraining_job_id: str = "",
        training_job_name: str = "",
        **kwargs,
    ) -> requests.Response:
        logger.debug(
            "modeltransform_create initiated with params:"
        ...
        data = {"modelTransformOutputS3Location": output_s3_location}
    ...
    def modeltransform_status(
        self, job_id: str, iam_role: str = ""
    ) -> requests.Response:
        data = {}
        if iam_role != "":
    ...
    def modeltransform_list(
        self, iam_role: str = "", max_items: int = 10
    ) -> requests.Response:
        data = {"maxItems": max_items}
    ...
    def modeltransform_stop(
        self, job_id: str, iam_role: str = "", clean: bool = False
    ) -> requests.Response:
    ...
    def endpoints_create(
        ...
        **kwargs,
        ...
        if model_training_job_id and not model_transform_job_id:
            ...
            raise ValueError(
                "Invalid input. Must either specify model_training_job_id or model_transform_job_id, "
                "and not both."
            )
    ...
    def endpoints_status(
        self, endpoint_id: str, neptune_iam_role_arn: str = ""
    ) -> requests.Response:
        data = (
            {}
            if neptune_iam_role_arn == ""
    ...
    def export(self, host: str, params: dict, ssl: bool = True) -> requests.Response:
        ...
        req = self._prepare_request(
            "POST", url, data=json.dumps(params), service="execute-api"
        )
        res = self._http_session.send(req, verify=self.ssl_verify)
        return res
    ...
    def _query_status(
        self, language: str, *, query_id: str = "", **kwargs
    ) -> requests.Response:
        data = {}
        if query_id != "":
    ...
    def statistics(
        self, language: str, summary: bool = False, mode: str = ""
    ) -> requests.Response:
